...
def kfolds_basic_test_all_classifiers(X_train, y_train, svm_type = 'rbf'):
    ...
    models = [svm.SVC(kernel=svm_type),
              ...
              AdaBoostClassifier(base_estimator=tree.DecisionTreeClassifier(random_state=0), n_estimators=100, random_state=0)
              ]
    ...
    for i in models:
        ...
        KFold_Score[classifiers[j]] = (cross_val_score(model, X_train, np.ravel(y_train), scoring='balanced_accuracy', cv=cv))
        j = j+1
    ...
    return KFold_Score, classifiers
